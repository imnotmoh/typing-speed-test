...
canva_text_obj = []
for i in random.choices(word_list,k=50):
    x += (13 * len(i)) / 2
    if x > 470:
        x = (13 * len(i)) / 2
        y += 30
    canva_text_obj.append(canva.create_text(x,y,text=f"{i}",font=("Ariel", 20, "bold"),fill="black"))
    x += (15 * len(i) -(len(i))) / 2

canva.grid(row=0,column=0,columnspan=5,padx=20)
label = Label(text="type in here", pady=50)
label.grid(column=0,row=1)
text_input = Entry(width=40)
text_input.grid(column=1,row=1,columnspan=3,pady=50)
